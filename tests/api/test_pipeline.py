...
def test_pipeline_flags_needs_review_in_confidence_band():
    fake_result = PredictionResult(label="spam", confidence=0.5, probabilities={"spam": 0.5, "bilgilendirme": 0.5})
    pipeline = ClassificationPipeline(_FakeClassifier(fake_result))
...
def test_pipeline_does_not_flag_high_confidence():
    fake_result = PredictionResult(label="bilgilendirme", confidence=0.95, probabilities={"bilgilendirme": 0.95})
    pipeline = ClassificationPipeline(_FakeClassifier(fake_result))
...
def test_pipeline_otp_rule_match_short_circuits_the_ml_classifier():
    fake_result = PredictionResult(label="spam", confidence=0.9, probabilities={"spam": 0.9})
    ...
    assert result.prediction.label == "otp"
    assert result.prediction.confidence == 0.95
    assert classifier.calls == []  # rule fired, ML model never consulted


def test_pipeline_non_otp_text_reaches_the_ml_classifier():
    fake_result = PredictionResult(label="bilgilendirme", confidence=0.9, probabilities={"bilgilendirme": 0.9})
    classifier = _FakeClassifier(fake_result)
    pipeline = ClassificationPipeline(classifier)

    result = pipeline.process("msg-1", "Kargonuz dagitima cikmistir.")
    assert result.prediction.label == "bilgilendirme"
    assert classifier.calls == ["Kargonuz dagitima cikmistir."]
